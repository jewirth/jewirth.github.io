...
- ***NOTE: I'm not sure about the next steps. It worked in July 2018 but when repeating the process in March 2019, my system became  instable after installing the nvidia driver at this point. Might be due to the current version of the driver.***
...
- *If you have installed the nvidia drivers:* Open the NVIDIA X Server Settings
- *If you have installed the nvidia drivers:* Switch to GPU "Intel (Power Saving Mode)"
- Add ```acpi_rev_override=5``` to ```GRUB_CMDLINE_LINUX_DEFAULT``` in ```/etc/default/grub``` and run ```sudo update-grub```
- Running ```/usr/share/doc/acpi-call-dkms/examples/turn_off_gpu.sh``` will cool the cpu down some more degrees and the fans will stay quiet most of the time ...TODO: execute this at boot time! *NOTE: As of March 2019, after running this script my system hung when shutting down, so I had to power off by holding the power button.*
